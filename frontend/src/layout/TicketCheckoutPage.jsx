...
function TicketCheckoutPage() {
  ...
  const basePrice = kaBiletë ? Number(event.tickets[0].cmimi) : 0;
  ...
  const cmimiBiletes = `${basePrice.toFixed(2)} EUR`;
  const cmimiFinal = `${Number(finalPrice).toFixed(2)} EUR`;
          ...
          <div className="mt-4 border-t border-white/10 pt-4 space-y-2">
            ...
            {appliedCoupon ? (
              <div className="flex items-center justify-between text-sm text-emerald-300">
                <span>Zbritje ({appliedCoupon.coupon?.code || couponCode})</span>
                <span>-{discountAmount.toFixed(2)} EUR</span>
              </div>
            ) : null}
            <div className="flex items-center justify-between text-base font-semibold border-t border-white/5 pt-2">
              <span>Totali</span>
              <span className="text-rose-400 text-lg font-bold">{cmimiFinal}</span>
            </div>
          </div>
        </section>
            ...
            <>
              ...
              <form className="mt-3 flex gap-2" onSubmit={handleApplyCoupon}>
                <input
                  ...
                  className="h-11 flex-1 rounded-md border border-white/10 bg-[#111925] px-3 text-sm uppercase text-white outline-none focus:border-rose-400"
                />
                <button
                  type="submit"
                  disabled={validating || !couponCode.trim() || !kaBiletë}
              ...
              {appliedCoupon ? (
                <div className="mt-3 flex items-center justify-between rounded-md border border-emerald-400/30 bg-emerald-500/10 px-3 py-2 text-sm text-emerald-200">
                  <span>
                    {appliedCoupon.coupon?.code} applied — save {discountAmount.toFixed(2)} EUR
                  </span>
              ...
              <h2 className="mt-6 text-lg font-semibold tracking-wide text-white/90">
                Detajet e Kartelës Bankare
              </h2>
              <form className="mt-4 space-y-4" onSubmit={handleCheckout}>
                ...
                >
                  {submitting ? "Duke procesuar..." : `Paguaj ${cmimiFinal}`}
                </button>
              </form>
